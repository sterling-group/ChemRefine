...
- **PySCF**: `basis` is now required (and `xc` for `method: dft`) on both engines — no silent level of theory; the direct engine refuses at preflight, before anything runs;
