...
def build_array_script(
    *,
    ...
    derives ``$INP_NAME`` / ``$OUT_NAME``, and ``exec``-redirects itself to the
    canonical per-structure ``.runlog`` / ``.err`` so on-disk artifacts match
    the per-job path exactly. ``run_block`` must therefore reference the bash
    variables rather than literal filenames — the engine renders it against
    sentinel paths named ``$INP_NAME`` / ``$OUT_NAME``. Failures before the
    redirect land in the SBATCH fallback log ``array_%A_%a.log``.
    """
    sbatch_lines, body_lines = _read_header(template_path)
    ...
    resolution = [
        ...
        f'exec >"{output_dir}/${{INP_NAME%.*}}.runlog" 2>"{output_dir}/${{INP_NAME%.*}}.err"',
        "",
    ]
