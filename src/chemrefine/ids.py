...
def resolve_persistent_ids(
    ...
    * **Uneven fan-out** — falls back to giving the extras to parent 0
    ...
    if p == 1:
        ...
        # child_count > p and not evenly divisible. Each parent gets at
        # least 1 child (giving p - 1 children to parents 1..p-1). The
        # leftover is child_count - (p - 1), which all goes to parent 0,
        # keeping the most-important / lowest-energy structure's branch
        # largest while every other parent still gets exactly one child.
