...
def test_python_header_writes_file_with_all_required_fields(tmp_path: Path):
    ...
    # No ORCA-specific fields leak into direct-mode runlogs by default.
    assert "orca_executable=" not in text


def test_python_header_appends_extra_fields(tmp_path: Path):
    """Engine-supplied ``extra_fields`` render after the fixed runlog skeleton."""
    log = tmp_path / "step1_structure_0.runlog"
    job_log.python_header(
        engine="mlff-direct",
        operation="opt_sp",
        ...
        step_dir=tmp_path / "outputs" / "step1_screen",
        log_path=log,
        extra_fields=(("model", "uma-s-1"),),
    )
    assert "model=uma-s-1" in log.read_text(encoding="utf-8")
